...
from backend.database.connection import engine, normalize_database_url
...
with TestClient(app) as c:
    ...
    print(
        "users:", as_.status_code, [u["username"] for u in as_.json().get("items", [])]
    )
    ...
    print(
        ...
        s.status_code,
        "score=",
        s.json().get("security_score"),
        "events=",
        s.json().get("total_events"),
    )
    ...
    print(
        "ingest:", ingest.status_code, ingest.json().get("saved_events"), "events saved"
    )
