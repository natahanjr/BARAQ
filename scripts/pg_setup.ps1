...
function Invoke-Start {
    ...
    & $pgctl -D $Data -o "-p $Port -h $Bind" -l $Log start
    $deadline = [DateTime]::UtcNow.AddSeconds(90)
    while (-not (Test-ClusterUp) -and [DateTime]::UtcNow -lt $deadline) {
    ...
    if (-not (Test-ClusterUp)) {
        ...
        throw "cluster did not come up on $Bind`:$Port (see $Log)`n$tail"
    }
