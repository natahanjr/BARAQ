...
def _scan(
    patterns: list[tuple[str, re.Pattern]], roots: list[Path], skip: set[Path]
) -> list[tuple[str, str]]:
    ...
    if suspicious:
        RESULTS.append(
            (
                "secrets-in-repo",
                "FAIL",
                f"{len(suspicious)} suspicious hit(s): "
        ...
        RESULTS.append(
            (
                "secrets-in-repo",
                "PASS",
                "no secret-looking strings outside config/tests/docs",
            )
        )
    ...
    if (ROOT / "secrets.dat").exists():
        RESULTS.append(
            (
                "vault-present",
                "PASS",
                "secrets.dat present (DPAPI-encrypted credential store)",
            )
        )
    else:
        RESULTS.append(
            (
                "vault-present",
                "WARN",
                "secrets.dat missing - credentials will be regenerated",
            )
        )
...
def prod_gate():
    ...
    checks = {
        ...
        "docs-hidden-in-prod": "docs_url=None if IS_PRODUCTION" in main
        or "IS_PRODUCTION" in main,
    }
    ...
    if missing:
        ...
        RESULTS.append(
            (
                "prod-gate",
                "PASS",
                "IS_PRODUCTION + _assert_production_safe + docs hiding present",
            )
        )
...
def login_rate_limit():
    src = (ROOT / "backend" / "api" / "auth.py").read_text(
        encoding="utf-8", errors="ignore"
    )
    if "_check_login_rate_limit" in src and "429" in src:
        RESULTS.append(
            ("login-rate-limit", "PASS", "failed-login throttling + lockout present")
        )
    else:
...
@check("mfa")
def mfa():
    auth_src = (ROOT / "backend" / "api" / "auth.py").read_text(
        encoding="utf-8", errors="ignore"
    )
...
def backup_and_migrations():
    have = (ROOT / "scripts" / "db_backup.py").exists() and (
        ROOT / "alembic" / "versions"
    ).is_dir()
    if have:
        RESULTS.append(
            ("backup-and-migrations", "PASS", "db_backup.py + alembic present")
        )
    else:
        RESULTS.append(
            ("backup-and-migrations", "WARN", "backup/migration tooling missing")
        )
...
def deps_consistent():
    ...
    if not py.exists():
        RESULTS.append(
            ("deps-consistent", "WARN", "no venv at venv/Scripts/python.exe")
        )
        return
    proc = subprocess.run(
        [str(py), "-m", "pip", "check"], capture_output=True, text=True, cwd=ROOT
    )
    ...
    if proc.returncode != 0:
        RESULTS.append(
            (
                "cve-audit",
                "WARN",
                "pip-audit not installed (pip install pip-audit to enable CVE scanning)",
            )
        )
        return
    try:
        scan = subprocess.run(
            [str(py), "-m", "pip_audit", "-l"],
            capture_output=True,
            text=True,
            cwd=ROOT,
            timeout=30,
        ...
        RESULTS.append(
            ("cve-audit", "WARN", "pip-audit timed out (no network?) - rerun later")
        )
        return
...
def main() -> int:
    ...
    print("-" * 78)
    print(
        f"{len(RESULTS)} checks: {len(RESULTS) - len(fails) - len(warns)} pass, "
