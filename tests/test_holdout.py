...
def test_holdout_detects_unseen_attacks(db):
    ...
    for scenario, rule in SCENARIO_RULE.items():
        assert by_scenario[scenario][
            "rule_detected"
        ], f"rule {rule} did not fire on {scenario}"
    ...
    runs = (
        db.query(EvaluationRun).filter(EvaluationRun.scenario.like("holdout:%")).all()
    )
    ...
    covered_scenarios = {
        s["scenario"] for s in result["per_scenario"] if s["hybrid_tp"] > 0
    }
...
def test_holdout_domain_randomization_is_seeded_and_stable(db):
    ...
    a = run_holdout_evaluation(
        db, with_ml=False, use_real_baseline=False, randomize=True, seed=42
    )
    b = run_holdout_evaluation(
        db, with_ml=False, use_real_baseline=False, randomize=True, seed=42
    )
    ...
    assert [r["hybrid_tp"] for r in a["per_scenario"]] == [
        r["hybrid_tp"] for r in b["per_scenario"]
    ]
    ...
    for scenario in HOLDOUT_SCENARIOS:
        ...
        assert (
            stats["rule_detected"] or stats["hybrid_tp"] > 0
        ), f"{scenario} missed under randomization"
