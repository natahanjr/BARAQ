...
def test_queue_command_requires_admin():
    ...
    with TestClient(app, headers={"X-API-Key": "sentinel-dev-analyst"}) as bare:
...
def test_pending_rejects_unknown_agent_key(client):
    resp = client.get("/api/commands/pending", headers={"X-Agent-Key": "sentinel-nope"})
    assert resp.status_code == 401
...
def test_full_command_lifecycle(client):
    ...
    pending = client.get(
        "/api/commands/pending", headers={"X-Agent-Key": "sentinel-agent-dev"}
    ).json()["items"]
    assert any(c["id"] == cmd["id"] for c in pending)

    wrong_agent = client.post(
        f"/api/commands/{cmd['id']}/result",
        headers={"X-Agent-Key": "sentinel-agent-laptop2"},
        json={"status": "success"},
    )
