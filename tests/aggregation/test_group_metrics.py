...
from tests.aggregation.helpers import GROUP_T0, fabricate_alerts
...
def test_metrics_all_fields(db):
    alerts = fabricate_alerts(
        ...
        [
            ...
            {"detector_id": "D002", "mitre": "T1110", "minutes_ago": 3.0},
            {
                "detector_id": "D003",
                "host": "finance-host",
                "user": "bob",
...
def test_evaluation_reports_raw_counts_not_fake_accuracy(db):
    ...
    assert (
        counts["correct_groupings"] + counts["incorrect_groupings"]
        == counts["labeled_groups"]
    )
