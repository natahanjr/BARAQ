...
def test_fingerprint_is_deterministic():
    ...
    assert group_fingerprint(a, "authentication") == group_fingerprint(b, "authentication")
...
def test_fingerprint_depends_on_host_user_source_family():
    ...
    assert group_fingerprint(detection(host="other", user="u", source_ip="1.1.1.1"),
                             "authentication") != base
    assert group_fingerprint(detection(host="ml-host", user="v", source_ip="1.1.1.1"),
                             "authentication") != base
    assert group_fingerprint(detection(host="ml-host", user="u", source_ip="2.2.2.2"),
                             "authentication") != base
    assert group_fingerprint(detection(host="ml-host", user="u", source_ip="1.1.1.1"),
                             "execution") != base
...
def test_fingerprint_is_never_a_random_uuid():
    ...
    assert len(fp) == 64
    assert not any(
        fp == uuid.uuid4().hex for _ in range(5)
    )
