...
def test_group_001_same_host_user_source_time_one_group(db):
    alerts = fabricate_alerts(
        db,
        [
            dict(minutes_ago=3.0),
            dict(detector_id="D002", mitre="T1110", minutes_ago=2.0),
            dict(detector_id="D001", minutes_ago=1.0),
        ],
    )
...
def test_group_002_same_host_different_user_separate_groups(db):
    alerts = make_alerts(
        db,
        [
            dict(user="alice", minutes_ago=2.0),
            dict(user="bob", minutes_ago=1.0),
        ],
    )
    process_alerts(db, alerts, now=GROUP_T0)
...
def test_group_003_same_user_different_host_separate_groups(db):
    alerts = make_alerts(
        db,
        [
            dict(host="ml-host", minutes_ago=2.0),
            dict(host="finance-host", minutes_ago=1.0),
        ],
    )
    process_alerts(db, alerts, now=GROUP_T0)
    assert len(stored_groups(db)) == 2
...
def test_group_005_outside_time_window_new_group(db):
    alerts = fabricate_alerts(
        ...
        [
            ...
            dict(minutes_ago=60.0),  # beyond the 15-minute auth window
        ],
    )
    process_alerts(db, alerts, now=GROUP_T0)
    groups = stored_groups(db)
...
def test_group_006_closed_group_later_alert_new_group(db):
    alerts = fabricate_alerts(db, [dict(minutes_ago=90.0)])
    process_alerts(db, alerts, now=GROUP_T0)
...
def test_group_007_same_mitre_technique_alone_does_not_group(db):
    alerts = make_alerts(
        db,
        [
            dict(detector_id="D001", mitre="T1133", host="h1", user="u1",
                 source_ip="1.1.1.1", minutes_ago=2.0),
            dict(detector_id="D003", mitre="T1133", host="h2", user="u2",
                 source_ip="2.2.2.2", minutes_ago=1.0),
        ],
    )
    process_alerts(db, alerts, now=GROUP_T0)
    assert len(stored_groups(db)) == 2
...
def test_group_011_cross_host_no_campaign_group(db):
    alerts = make_alerts(
        db,
        [
            dict(host="host-1", user="user-a", minutes_ago=2.0),
            dict(host="host-2", user="user-b", minutes_ago=1.0),
            dict(host="host-3", user="user-c", minutes_ago=0.5),
        ],
    )
    process_alerts(db, alerts, now=GROUP_T0)
...
def test_group_012_severity_does_not_escalate(db):
    alerts = make_alerts(
        db,
        [
            dict(severity="high", minutes_ago=2.0),
            dict(detector_id="D002", mitre="T1110", severity="high", minutes_ago=1.0),
        ],
    )
    process_alerts(db, alerts, now=GROUP_T0)
    group = stored_groups(db)[0]
...
def test_do_d_30_related_alerts_one_group(db):
    ...
    specs = [dict(detector_id="D001", minutes_ago=i * 0.4) for i in range(30)]
...
def test_do_d_unrelated_activity_stays_separate(db):
    alerts = make_alerts(
        ...
        [
            dict(host="host-a", user="user-a", minutes_ago=2.0),
            dict(detector_id="D005", host="host-b", user="user-b",
