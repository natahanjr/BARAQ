...
def _summary(**overrides):
    base = {
        "id": "BG-000001", "family": "authentication",
        "hosts": ["host-a"], "users": ["alice"], "sources": ["203.0.113.5"],
        "destinations": ["10.0.0.9"], "techniques": ["T1110"],
...
def test_same_host_user_source_and_temporal():
    ...
    b = _summary(id="BG-000002", first_seen=T0 + __import__("datetime").timedelta(minutes=5))
...
def test_destination_relation_and_network_relation():
    ...
    b = _summary(id="BG-000002", hosts=["10.0.0.9"], destinations=["10.0.0.9"],
...
def test_no_relationship_without_shared_context():
    a = _summary(id="BG-000001", hosts=["host-a"], users=["alice"], sources=["1.1.1.1"],
                 destinations=["10.0.0.9"], first_seen=T0)
    b = _summary(id="BG-000002", hosts=["host-b"], users=["bob"], sources=["2.2.2.2"],
                 destinations=["10.0.0.8"], first_seen=T0 + __import__("datetime").timedelta(minutes=5))
    rel = pair_relationships(a, b, window_key="multi_stage", within_window=None)
    assert "TEMPORAL" in rel["types"]
...
def test_technique_transition_only_within_family():
    ...
    c = _summary(id="BG-000003", family="execution", techniques=["T1059.001"],
                 first_seen=T0)
...
def test_edge_strength_is_capped_and_deterministic():
    ...
    assert edge_strength(
        ["SAME_HOST", "SAME_USER", "SAME_SOURCE", "TEMPORAL", "TECHNIQUE_TRANSITION"]
    ) == 1.0
...
def test_temporal_requires_window():
    a = _summary(id="BG-000001", first_seen=T0)
    b = _summary(id="BG-000002", first_seen=T0 + __import__("datetime").timedelta(minutes=200))
