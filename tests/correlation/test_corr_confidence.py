...
def test_bounded_between_min_and_max():
    for value in (
        ...
        confidence({"SAME_HOST", "SAME_USER", "SAME_SOURCE", "TEMPORAL"}, 2, True, True),
        confidence({"A", "B", "C", "D", "E", "F", "G", "H"}, 9, True, True),
    ):
        assert config.CORRELATION_CONFIDENCE_MIN <= value <= config.CORRELATION_CONFIDENCE_MAX
...
def test_canonical_example_is_exactly_0_88():
    ...
    value = confidence(
        {"SAME_USER", "SAME_SOURCE", "TEMPORAL", "DESTINATION_RELATION", "LATERAL_MOVEMENT"},
        chain_length=4,
...
def test_formula_deterministic():
    args = ({"SAME_USER", "SAME_SOURCE", "TEMPORAL", "DESTINATION_RELATION"}, 3, True, True)
