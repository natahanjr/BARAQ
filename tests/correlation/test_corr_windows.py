...
def test_window_is_bounded():
    ...
    assert not within_window(T0, T0 + timedelta(minutes=31, seconds=1), "authentication_to_execution")
    assert within_window(T0, T0 + timedelta(minutes=59), "execution_to_privilege")
    assert not within_window(T0, T0 + timedelta(minutes=61, seconds=1), "execution_to_privilege")
...
def test_clock_drift_slack_is_bounded():
    ...
    assert within_window(T0, T0 + timedelta(minutes=30, seconds=59), "authentication_to_execution")
    assert not within_window(T0, T0 + timedelta(minutes=30, seconds=61), "authentication_to_execution")
