...
SCENARIOS: list[dict] = [
    {
        ...
        "expected": {
            "HOST:h001": {
                "score": 11.0, "severity": "MINIMAL", "state": "NORMAL",
    ...
    },
    {
        ...
        "expected": {
            "HOST:h002": {
                "score": 42.0, "severity": "MEDIUM", "state": "HIGH",
                "alert_count": 10,
                "factors": ["RF003_LATERAL_MOVEMENT", "RF010_BEHAVIOR_GROUP",
    ...
    },
    {
        ...
        "expected": {
            "HOST:h003": {
                "score": 48.0, "severity": "MEDIUM", "state": "HIGH",
                "alert_count": 10, "group_count": 1, "correlation_count": 1,
                "factors": ["RF002_CREDENTIAL_ACCESS", "RF010_BEHAVIOR_GROUP",
                            "RF009_ALERT_SEVERITY", "RF008_RECENCY",
    ...
    },
    {
        ...
        "steps": [
            {"at": T0, "evidence": [
                _group("g-f", "h005", ["T1021.001"],
                       observed=T0 - timedelta(hours=24)),
            ]},
        ],
        "expected": {
            "HOST:h005": {
                "score": 17.0, "severity": "MINIMAL", "state": "NORMAL",
                "factors": ["RF003_LATERAL_MOVEMENT", "RF010_BEHAVIOR_GROUP",
                            "RF009_ALERT_SEVERITY"],
            },
        },
    },
    {
        ...
        "steps": [
            {"at": T0, "evidence": [
                _group("g-g", "h006", ["T1021.001"],
                       observed=T0 - timedelta(hours=200)),
            ]},
            {"at": T0, "expire": True},
        ],
        "expected": {
            "HOST:h006": {
                "score": 0.0, "severity": "MINIMAL", "state": "NORMAL",
                "factors": ["RF003_LATERAL_MOVEMENT", "RF010_BEHAVIOR_GROUP",
                            "RF009_ALERT_SEVERITY"],
            },
        },
    },
    {
        ...
        "expected": {
            "HOST:h007": {
                "score": 43.0, "severity": "MEDIUM", "state": "HIGH",
                "factors": ["RF009_ALERT_SEVERITY", "RF007_REPETITION", "RF008_RECENCY"],
                "repetition_count": 4,
            },
        },
    },
    {
        ...
        "steps": [
            ...
            {"at": T0, "propagate": {
                "target_entity_type": "USER", "target_entity_id": "u008a",
                "from_entity": "HOST:h008a", "relationship_type": "host_to_user",
                "reason": "user accounts on a high-risk host",
            }},
        ],
        "expected": {
            "USER:u008a": {
                "score": 16.0, "severity": "MINIMAL", "state": "NORMAL",
                "confidence": 0.5,
    ...
    },
    {
        ...
        "expected": {
            "HOST:h010a": {
                "score": 66.0,
                "factors": ["RF003_LATERAL_MOVEMENT", "RF002_CREDENTIAL_ACCESS",
                            "RF010_BEHAVIOR_GROUP", "RF009_ALERT_SEVERITY",
                            "RF008_RECENCY"],
            },
            "HOST:h010b": {
                "score": 66.0,
    ...
    },
    {
        ...
        "steps": [
            ...
            {"at": T0 + timedelta(hours=24),
             "recalculate_entities": ["HOST:h012"]},
        ],
        "expected": {
            "HOST:h012": {
                "score": 17.0, "severity": "MINIMAL", "state": "STALE",
                "peak_score": 42.0, "trend": "FALLING",
            },
        },
    },
    {
        ...
        "expected": {
            "HOST:h013": {
                "score": 45.0, "severity": "MEDIUM", "state": "HIGH",
                "crossed": ["LOW", "MEDIUM"],
            },
        },
    },
    {
        ...
        "steps": [
            ...
            {"at": T0 + timedelta(hours=2),
             "recalculate_entities": ["HOST:h014"]},
        ],
        "expected": {
    ...
    },
    {
        ...
        "steps": [
            ...
            {"at": T0 + timedelta(hours=24),
             "recalculate_entities": ["HOST:h015b"]},
        ],
        "expected": {
    ...
    },
    {
        ...
        "expected": {
            "HOST:h016": {
                "score": 52.0, "severity": "MEDIUM", "state": "HIGH",
                "factors": ["RF003_LATERAL_MOVEMENT", "RF010_BEHAVIOR_GROUP",
                            "RF009_ALERT_SEVERITY", "RF008_RECENCY",
                            "RF006_MULTI_STAGE_CORRELATION"],
            },
    ...
    },
    {
        ...
        "steps": [
            {"at": T0, "evidence": [
                {
                    "kind": "BEHAVIOR_GROUP",
                    "group_id": "g-p", "hosts": ["h017"], "users": ["u017"],
            ...
            {"at": T0, "propagate": {
                "target_entity_type": "HOST", "target_entity_id": "h017",
                "from_entity": "HOST:x", "relationship_type": "suspicious",
                "reason": "suspicious link", "expect_error": True,
            }},
        ],
        "expected": {
            "HOST:h017": {
                "score": 24.0,
                "factors": ["RF010_BEHAVIOR_GROUP", "RF009_ALERT_SEVERITY",
                            "RF008_RECENCY"],
            },
        },
    },
    {
        ...
        "steps": [
            ...
            {"at": T0, "propagate": {
                "target_entity_type": "HOST", "target_entity_id": "h018",
                "from_entity": "USER:u018", "relationship_type": "user_to_host",
                "reason": "host used by a user present elsewhere",
            }},
        ],
        "expected": {
            "HOST:h018": {
                "score": 50.0, "confidence": 0.84,
                "factors": ["RF003_LATERAL_MOVEMENT", "RF010_BEHAVIOR_GROUP",
                            "RF009_ALERT_SEVERITY", "RF008_RECENCY",
                            "RF006_MULTI_STAGE_CORRELATION"],
            },
    ...
    },
    {
        ...
        "expected": {
            "HOST:h019": {
                "score": 42.0, "model_version": "1.0.0",
                "factors": ["RF003_LATERAL_MOVEMENT", "RF010_BEHAVIOR_GROUP",
                            "RF009_ALERT_SEVERITY", "RF008_RECENCY"],
            },
        },
    },
    {
        ...
        "expected": {
            ...
            "isolation": ["v2_alerts", "behavior_groups", "correlation_findings",
                          "incidents"],
        },
    },
    {
        ...
        "steps": [
            {"at": T0, "evidence": [
                _group("g-t1", "h021", ["T1021.001"], user="u021", source="203.0.113.21"),
                _group("g-t2", "h021b", ["T1110"], user="u021", source="203.0.113.21"),
            ...
            {"at": T0, "evidence": [
                {
                    ...
                    "detector_id": "D100",
                    "host": "h023", "user": "u023",
                    "source_ip": "203.0.113.23", "destination_ip": "10.0.0.23",
                    "account": "svc-023", "process": "powershell.exe",
                    "severity": "medium",
                    "first_seen": T0, "last_seen": T0,
                },
            ]},
    ...
    },
    {
        ...
        "expected": {
            "HOST:h024": {"score": 42.0, "audit_actions": [
                "RISK_CREATED", "FACTOR_ADDED", "RISK_RECALCULATED",
            ]},
        },
    },
    {
    ...
    },
    {
        ...
        "expected": {
            "HOST:h026": {
                "score": 9.0, "severity": "MINIMAL", "state": "NORMAL",
                "factors": ["RF009_ALERT_SEVERITY", "RF008_RECENCY"],
            },
        },
    },
    {
        ...
        "expected": {
            "HOST:h027": {
                "score": 14.0, "severity": "MINIMAL", "state": "NORMAL",
                "factors": ["RF009_ALERT_SEVERITY", "RF008_RECENCY"],
            },
        },
    },
