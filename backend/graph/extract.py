...
def sync_graph(db, store: GraphStore) -> dict:
    ...
    for r in rows:
        nodes.append(
            _node(
                ...
                r.host,
                max(r.mx or 0, 0),
                label=f"Device: {r.host}",
                events=r.c,
                props={"hostname": r.host},
                first=r.mn,
                last=r.mx_ts,
            )
        )
    ...
    rows = db.execute(
        ...
        .where(
            NormalizedEvent.user.isnot(None),
            NormalizedEvent.user != "-",
            NormalizedEvent.host.isnot(None),
            NormalizedEvent.host != "-",
        )
    ...
    for r in rows:
        nodes.append(
            _node(
                ...
                r.name,
                0.0,
                label=f"Process: {r.name}",
                events=r.c,
                props={"process_name": r.name},
                first=r.mn,
                last=r.mx_ts,
            )
        )
    ...
    for r in rows:
        ...
        nodes.append(
            _node(
                ...
                r.remote_ip,
                risk,
                label=f"IP: {r.remote_ip}",
                events=r.c,
                props={"ip": r.remote_ip},
                first=r.mn,
                last=r.mx_ts,
            )
        )
    ...
    rows = db.execute(
        ...
        .where(
            NetworkConnection.process.isnot(None),
            NetworkConnection.process != "",
            NetworkConnection.remote_ip.isnot(None),
            NetworkConnection.remote_ip != "",
        )
    ...
    for r in rows:
        ...
        nodes.append(
            _node(
                ...
                r.query,
                risk,
                label=f"Domain: {r.query}",
                events=r.c,
                props={"domain": r.query},
                first=r.mn,
                last=r.mx_ts,
            )
        )
    ...
    rows = db.execute(
        select(
            FileScan.sha256,
            FileScan.file_name,
            FileScan.is_malicious,
            func.count().label("c"),
            func.min(FileScan.scanned_at).label("mn"),
    ...
    for r in rows:
        ...
        nodes.append(
            _node(
                ...
                r.sha256,
                risk,
                label=r.file_name or f"File: {r.sha256[:12]}...",
                events=r.c,
                props={"sha256": r.sha256, "file_name": r.file_name},
                first=r.mn,
                last=r.mx_ts,
            )
        )
    ...
    rows = db.execute(
        select(
            ...
            Alert.mitre_name,
            func.count().label("c"),
            func.min(Alert.created_at).label("mn"),
            func.max(Alert.created_at).label("mx_ts"),
        )
        .where(
            Alert.mitre_id.isnot(None), Alert.mitre_id != "", Alert.mitre_id != "T0000"
        )
    ...
    for r in rows:
        nodes.append(
            _node(
                ...
                r.mitre_id,
                40.0,
                label=f"{r.mitre_id} {r.mitre_name}".strip(),
                alerts=r.c,
                events=0,
                props={"mitre_id": r.mitre_id, "mitre_name": r.mitre_name},
                first=r.mn,
                last=r.mx_ts,
            )
        )
    ...
    alert_techniques = {
        ...
        if a.mitre_id and a.mitre_id != "T0000"
    }
...
def ingest_batch(
    db, store: GraphStore, records: list[dict], alerts: list[Alert]
) -> None:
    ...
    for rec in records:
        ...
        if source == "process" and rec.get("name"):
            nodes.append(
                _node("process", rec["name"], 0.0, label=f"Process: {rec['name']}")
            )
            if user:
            ...
            if q:
                ...
                if rec.get("response"):
                    nodes.append(
                        _node(
                            "ip", rec["response"], 0.0, label=f"IP: {rec['response']}"
                        )
                    )
                    edges.append(
                        _edge("domain", q, "RESOLVES_TO", "ip", rec["response"])
                    )
        if user and user != "-":
        ...
        if mitre and mitre != "T0000":
            nodes.append(
                _node(
                    ...
                    alerts=1,
                    props={"mitre_id": mitre, "mitre_name": alert.mitre_name},
                )
            )
            if alert.host and alert.host != "-":
                edges.append(
                    _edge("device", alert.host, "EXHIBITS", "technique", mitre)
                )
...
def _attribute_alert_actors(db, store: GraphStore, alerts: list[Alert]) -> None:
    ...
    try:
        ...
        items: list[dict] = []
        for alert in alerts:
            for ind in extract_indicators(
                f"{alert.evidence or ''} {alert.name or ''}", limit=6
            ):
