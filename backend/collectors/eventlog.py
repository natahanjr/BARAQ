...
class WindowsEventLogCollector(BaseCollector):
    ...
    def __init__(
        self, channels: list[str] | None = None, extra_event_ids: set[int] | None = None
    ):
        super().__init__()
    ...
    def _relevant(self, event_id: int) -> bool:
        return (
            event_id in self.INTERESTING_EVENT_IDS or event_id in self._extra_event_ids
        )
    ...
    def _structured_fields(self, channel: str, record_number: int) -> dict:
        ...
        try:
            ...
            handle = win32evtlog.EvtQuery(
                channel, win32evtlog.EvtQueryChannelPath, query
            )
        except Exception as exc:
            logger.debug(
                "structured query failed for %s/%s: %s", channel, record_number, exc
            )
            return {}
        try:
            ...
            logger.debug(
                "structured render failed for %s/%s: %s", channel, record_number, exc
            )
            return {}
        finally:
            try:
    ...
    def _parse_data_xml(xml: str) -> dict:
        ...
        for m in re.finditer(
            r"<Data\s+Name=[\"']([^\"']+)[\"']>(.*?)</Data>", xml, re.DOTALL
        ):
    ...
    def _collect_channel(self, channel: str) -> list[dict]:
        handle, _flags = self._open_channel(channel)
        out: list[dict] = []
        try:
            ...
            for event in events:
                ...
                if rec:
                    ...
                    if rec["event_id"] in STRUCTURED_FIELDS_EVENT_IDS:
                        structured = self._structured_fields(
                            channel, event.RecordNumber
                        )
                        if structured:
