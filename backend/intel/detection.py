...
def annotate_alert_intel(db, alert, offline: bool = True) -> dict | None:
    ...
    try:
        ...
        for indicator in indicators:
            ...
            except Exception:  # noqa: BLE001 - one bad lookup must not kill the batch
                logger.debug("Intel lookup failed for %s", indicator, exc_info=True)
                continue
            if verdict.get("category", "unknown") != "unknown" or (verdict.get("confidence") or 0) > 0:
...
def intel_hits(payload: dict | None) -> int:
    ...
    if not payload:
        return 0
    return sum(
        1 for v in payload.get("indicators", [])
