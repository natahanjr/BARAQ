...
def _db_size_bytes(session=None) -> float:
    ...
    try:
        with session or __import__(
            "backend.database.connection", fromlist=["SessionLocal"]
        ).SessionLocal() as db:
            return float(
...
def collect_metrics(session=None) -> str:
    ...
    try:
        ...
        for name, help_, model in (
            ...
            (
                "baraq_network_connections_total",
                "Network connection records persisted.",
                NetworkConnection,
            ),
            ("baraq_dns_queries_total", "DNS query records persisted.", DnsQuery),
            (
                "baraq_http_requests_total",
                "HTTP request records persisted.",
                HttpRequest,
            ),
        ...
        alert_rows = session.execute(
            select(
                Alert.org, Alert.severity, Alert.status, func.count(Alert.id)
            ).group_by(Alert.org, Alert.severity, Alert.status)
        ).all()
        if alert_rows:
            for org, severity, status, n in alert_rows:
                _emit(
                    ...
                    "counter",
                    "Alerts created, labelled by org, severity and status.",
                    {"org": org or "system", "severity": severity, "status": status},
                    n,
                )
        else:
            _emit(
                ...
                "counter",
                "Alerts created, labelled by org, severity and status.",
                {"org": "system", "severity": "none", "status": "none"},
                0,
            )
        ...
        open_alerts = int(
            session.scalar(select(func.count(Alert.id)).where(Alert.status == "open"))
            or 0
        )
        _emit(
            "baraq_open_alerts_total",
            "gauge",
            "Current number of open alerts (all orgs).",
            None,
            open_alerts,
        )
        _emit(
            "baraq_incidents_total",
            "gauge",
            "Incidents created.",
            None,
            _count(Incident),
        ...
        try:
            ...
            score = 0.0
        _emit(
            "baraq_security_score",
            "gauge",
            "Current host security score (0..100).",
            None,
            score,
        )
        ...
        _emit(
            "baraq_rules_total",
            "gauge",
            "Number of detection rules registered.",
            None,
            len(rules),
        )
        ...
        ready = [
            b for b in ("login", "process", "network") if b in (detector.models or {})
        ]
        _emit(
            "baraq_ml_streams_ready",
            "gauge",
            "ML behavior streams with a trained model.",
            None,
            len(ready),
        )

        _emit(
            "baraq_collectors_enabled", "gauge", "Collection sources active (0/1 each)."
        )
        manager = CollectorManager()
        for collector in manager.collectors:
            _emit(
                "baraq_collectors_enabled",
                "gauge",
                "Collection sources active (0/1 each).",
                {"collector": collector.name},
                1.0 if collector.enabled() else 0.0,
            )

        _emit(
            "baraq_uptime_seconds",
            "gauge",
