...
_LOLBIN_FLAGS = (
    (
        r"\brundll32(?:\.exe)?\b[^\n]*?(javascript:|vbscript:|https?://|\.hta\b)",
        "rundll32 script/remote execution",
    ),
    (
        r"\bmshta(?:\.exe)?\b[^\n]*?(javascript:|vbscript:|https?://|\.hta\b|ftp://)",
        "mshta executing script or remote .hta",
    ),
    (
        r"\bregsvr32(?:\.exe)?\b[^\n]*?/s\b[^\n]*?(https?://|scrobj|\.sct\b)",
        "regsvr32 squiblydoo (scrobj/remote .sct)",
    ),
    (r"\bregsvr32(?:\.exe)?\b[^\n]*?/i:[^\s]*?https?://", "regsvr32 remote scriptlet"),
    (
        r"\bcertutil(?:\.exe)?\b[^\n]*?(-urlcache|-decode|/decode|-ping|-split)",
    ...
    (
        r"\bw(?:script|cscript)\.exe\b[^\n]*?(\\temp\\|\\users\\public|\\appdata|\\downloads)",
        "script host executing from user-writable path",
    ),
)
...
class LolBinExecutionRule(BaseRule):
    ...
    def evaluate(self, window_minutes: int) -> list[DetectionResult]:
        ...
        for cmdline, label, user in self.cmdline_candidates(since):
            flags = [
                flag
                for pattern, flag in _LOLBIN_FLAGS
