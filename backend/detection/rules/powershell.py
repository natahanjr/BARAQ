...
SUSPICIOUS_PATTERNS = {
    "encoded_command": re.compile(
        r"-enc(?:odedcommand)?\b|FromBase64String", re.IGNORECASE
    ),
    ...
    "hidden_execution": re.compile(
        r"-W(indowStyle)?\s+Hidden\b|-w\s+hidden\b", re.IGNORECASE
    ),
    "reflective_invoke": re.compile(
        r"Invoke-Expression|iex\b|IEX\s|Invoke-Mimikatz|Get-KerberosTicket",
        re.IGNORECASE,
    ),
}
...
class SuspiciousPowerShellRule(BaseRule):
    ...
    def evaluate(self, window_minutes: int) -> list[DetectionResult]:
        ...
        for event in rows:
            script = (
                event.raw_json.get("facts", {}).get("script_block", "")
                if event.raw_json
                else ""
            )
            command_line = (
                event.raw_json.get("facts", {}).get("command_line", "")
                if event.raw_json
                else ""
            )
            ...
            if event.event_id == 4688 and not re.search(
                r"powershell|pwsh", haystack, re.IGNORECASE
            ):
                continue
            ...
            if "encoded_command" in hits or (
                "download_cradle" in hits and "hidden_execution" in hits
            ):
                severity = "critical"
