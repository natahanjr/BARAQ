...
class ContextFacts:
    ...
    def severity_adjust(self, confidence: float | None = None) -> str | None:
        """One-step demotion for dev-sensitive rules under strong dev context
        and low confidence - the classic analyst-laptop false positive."""
        if self.rule not in DEV_SENSITIVE_RULES:
            return None
