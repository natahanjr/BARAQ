  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-6">
      <div className="flex items-start gap-3">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-cyan-500/10 text-[12px] font-bold text-cyan-400 ring-1 ring-cyan-500/20">
          {n}
        </span>
        <div className="min-w-0 flex-1">
          <h3 className="text-[15px] font-bold text-white">{title}</h3>
          <div className="mt-2 text-[13px] leading-relaxed text-slate-400/80">{children}</div>
        </div>
      </div>
    </div>
  );
...
function Code({ children }) {
  return (
    <pre
      ...
      style={{ background: "#0f172a", borderColor: "rgba(255,255,255,0.04)", color: "#a5f3fc" }}
    >
      {children}
    </pre>
  );
    ...
    <div className="space-y-6 pb-12">
      <div className="rounded-3xl border border-white/[0.06] bg-white/[0.025] p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[24px] font-bold tracking-[-0.03em] text-white">Agent Setup</h1>
            <p className="mt-1 text-[13px] text-slate-400/80">Deploy BARAQ agents to remote endpoints</p>
          </div>
          <span className="inline-flex items-center gap-2 rounded-xl border border-cyan-500/20 bg-cyan-500/[0.06] px-3.5 py-2 text-[12px] text-cyan-300">
      ...
      <Step n={2} title="Provision a host key">
        ...
        <p className="mt-3 text-[11px] text-slate-500/60">
      ...
      <Step n={5} title="Remote actions (analyst-driven)">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {AGENT_ACTIONS.map((a) => (
            ...
            >
              <p className="font-mono text-[12px] font-semibold text-cyan-400">{a.action}</p>
      ...
      <div className="rounded-2xl border border-emerald-500/15 bg-gradient-to-br from-emerald-500/[0.04] to-transparent p-6">
        <h3 className="text-[15px] font-bold text-white">Direct ingestion</h3>
