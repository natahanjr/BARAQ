...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      {toast && (
        <div
          className="toast-in fixed right-4 top-4 z-[80] flex items-center gap-2.5 rounded-xl border px-5 py-3.5 shadow-lg sm:right-6 sm:top-6"
          style={{ background: "var(--notice-bg, #fff)", borderColor: "var(--notice-border, #e2e8f0)", color: "var(--notice-text, #1e293b)" }}
        >
          <span
            className="h-2.5 w-2.5 shrink-0 rounded-full"
            style={{ background: toast.kind === "success" ? "#10b981" : "#f59e0b" }}
          />
          <p className="text-sm font-medium" style={{ color: "var(--notice-text, #1e293b)" }}>{toast.text}</p>
        </div>
      )}
    </div>
  );
